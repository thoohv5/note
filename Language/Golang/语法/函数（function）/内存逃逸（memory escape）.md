...
Go 编译器内置了逃逸分析，它可以帮助开发者检测内存逃逸。你可以使用 `go build` 命令的 `-gcflags` 标志来启用逃逸分析并输出逃逸分析的结果。
...
[go中的内存逃逸 - 落雷 - 博客园](https://www.cnblogs.com/lianshuiwuyi/p/17804635.html)